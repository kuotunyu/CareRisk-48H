...
3. `test_gradio_contract_monkeypatch_loads_are_exact` — 17 rows;
4. `test_gradio_contract_sys_platform_contexts_are_exact` — 6 rows.

Each row asserts its new deterministic category tag (`protected_load:<name>`, `semantic_dunder:<name>`, `monkeypatch:load`, or `sys:context`) in both source and helper findings. On rejected candidate `84e2eaa...`, run exactly these four nodes: expected 73 collected and 73 failed. After implementation, run the same exact nodes: expected 73 collected and 73 passed. Do not count an unrelated existing finding as closure.
