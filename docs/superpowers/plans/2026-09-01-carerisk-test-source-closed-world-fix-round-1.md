...
Use exactly four new parameterized regression node IDs:

1. `test_gradio_contract_protected_identity_loads_are_exact` — 22 rows;
2. `test_gradio_contract_semantic_dunder_bindings_are_denied` — 28 rows;
3. `test_gradio_contract_monkeypatch_loads_are_exact` — 25 rows;
4. `test_gradio_contract_sys_platform_contexts_are_exact` — 6 rows.

Each row asserts its new deterministic category tag (`protected_load:<name>`, `semantic_dunder:<name>`, `monkeypatch:load`, or `sys:context`) in both source and helper findings. On rejected candidate `84e2eaa...`, run exactly these four nodes: expected 81 collected and 81 failed. After implementation, run the same exact nodes: expected 81 collected and 81 passed. Do not count an unrelated existing finding as closure.
...
11. `monkeypatch`: permit a load as the immediate receiver Name of an approved exact direct `monkeypatch.setattr(...)` call or the sole exact `setenv` call. Also permit exactly eight helper arguments: argument index 2 of `make_unit_failure_bundle` in `test_app_owned_root_is_not_a_nested_main_landmark`, `test_failure_page_is_one_static_document_with_no_capabilities`, and `test_schema_failure_controlled_seam_has_exact_copy_and_no_partial_surface`; and argument index 0 of `_fake_asset_roots` in `test_package_asset_file_or_directory_symlink_fails_closed`, `test_package_asset_containment_escape_fails_closed`, `test_package_asset_special_file_fails_closed`, `test_package_asset_duplicate_url_fails_closed`, and `test_package_asset_case_alias_fails_closed`. Require exact owner/callee/argument position and cardinalities 3+5.

Protect the two helper destinations: each is one unique top-level undecorated, non-generic FunctionDef with its exact current signature and return annotation. `make_unit_failure_bundle` has parameter index 2 exactly `monkeypatch: pytest.MonkeyPatch`; its only parameter loads are the receivers of the two current direct `setattr` calls for `RECEIPT_SHA256` and `RECEIPT_GIT_BLOB_SHA`. `_fake_asset_roots` has parameter index 0 exactly `monkeypatch: pytest.MonkeyPatch`; its only parameter loads are the receivers of the two current direct `setattr` calls for `BUILD_PATH_LIB` and `STATIC_PATH_LIB`. Each helper name can load only as the callee of its enumerated 3 or 5 exact calls. Reject decorators, changed header/parameter/annotation, alias/import/rebind/delete/global/nonlocal, wrong internal method/receiver/member/count, wrong callee/position/owner, duplicate/extra occurrence, return/capture, method reference, assignment, alternate receiver, or unapproved method. The eight additional RED rows cover both helpers' decorator, rebind, renamed-parameter, and alternate-method variants.
