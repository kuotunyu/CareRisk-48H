...
- Reject every `ast.arg` named exactly `request` or `pytestconfig`, regardless of whether it belongs to a function, fixture, or collected class method. Preserve current nested `async def request`, `RunningWireApp.request`/`requests` method names, `request_bytes`, and local/loop variables named `request`, none of which is an exact `ast.arg` with a forbidden name. Add a collected `TestEscape.test_escape(self, request)` RED that attempts `request.module.importlib.reload(ui_module)`.
- Permit exactly one first positional parameter `entrypoint: Any` owned by the sole top-level helper `_assert_entrypoint_positional_identity`. Require one undecorated, non-generic top-level FunctionDef with its exact current signature and exact complete five-statement body: five direct `ast.Assert` nodes in current order for parent identity, demo identity, app identity, exact guard `isinstance`, and downstream-parent identity. No wrapper, extra statement, reordering, deletion, or alternate expression is allowed. Its helper-name loads are exactly the two current direct `Call.func` nodes in `test_entrypoint_mount_and_uvicorn_contract_are_exact`. Add REDs for wrong owner/position/annotation/header, decorator, duplicate helper/parameter, added load/call/return, alternate attribute/call site, nested `if False`, nested `try`, reordered/deleted/changed assert, alias/import, and any local/global/nonlocal/delete/rebind. This helper parameter is disjoint from the protected dynamic-module local.
...
- reject receiver-independently every attribute/member token `importorskip`, `importer`, `import_from_string`, `resolve_name`, `locate`, `find_spec`, `import_plugin`, `load_setuptools_entrypoints`, or `pluginmanager`, plus original/effective imports or bindings of those names. Do not infer whether the call would import; reject the source token. Permit a `pytest` Name load only as the exact receiver root for bare `@pytest.fixture`, exact current `@pytest.fixture(scope="module")`, existing `mark.parametrize`, `raises`, `skip`, `fail`, and annotation-only `MonkeyPatch`/`TempPathFactory` contexts; reject every other pytest attribute, including `main`/`console_main`. Reject every `ast.arg` named exactly `request` or `pytestconfig` while preserving non-argument names and locals;
...
For implementation, the actual-source corrective rules above supersede the broader phrases "reject semantic bindings `request`/`pytestconfig`" and "reject every other semantic binding of ... `entrypoint`". Reject exact forbidden argument names everywhere but preserve non-argument request names; the only `entrypoint` exception is the exact protected `_assert_entrypoint_positional_identity` FunctionDef/signature/five direct-body asserts/two direct call sites enumerated above. Compare the helper body as an exact ordered canonical AST sequence so nested control flow cannot retain a matching descendant while suppressing execution. All near misses fail. The exact `for name, value in {...}.items()` target is an `ast.Tuple` in `Store` context and must be compared with that context rather than a `Load` tuple.
