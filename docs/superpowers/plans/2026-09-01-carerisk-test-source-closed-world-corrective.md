...
- require the module-level `Import`/`ImportFrom` nodes to equal the exact current 34-node canonical AST sequence, preserving order, original names, aliases, members, and level; reject every nested import. Independently reject every original/effective token or semantic binding named `unittest`, `mock`, `patch`, `pytest_mock`, or `mocker`, and every attribute token named `patch`;
