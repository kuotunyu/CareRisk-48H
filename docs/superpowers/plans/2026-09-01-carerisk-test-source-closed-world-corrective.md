...
- reject every forbidden builtin `Name` load except the two recorded `getattr` nodes and the one recorded `type` node; `breakpoint` is a forbidden builtin with no exception, and every semantic binding/import original/effective alias of that name also fails;
