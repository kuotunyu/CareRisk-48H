...
- Permit exactly one first positional parameter `entrypoint: Any` owned by the sole top-level helper `_assert_entrypoint_positional_identity`. Require one undecorated, non-generic top-level FunctionDef with its exact current signature. Its only `entrypoint` Name loads are exact receivers in `mounted_parent is entrypoint.parent`, `mounted_demo is entrypoint.demo`, and `served_app is entrypoint.app`; its helper-name loads are exactly the two current direct `Call.func` nodes in `test_entrypoint_mount_and_uvicorn_contract_are_exact`. Add REDs for wrong owner/position/annotation/header, decorator, duplicate helper/parameter, added load/call/return, alternate attribute/call site, alias/import, and any local/global/nonlocal/delete/rebind. This helper parameter is disjoint from the protected dynamic-module local.
...
For implementation, the actual-source corrective rules above supersede the broader phrases "reject semantic bindings `request`/`pytestconfig`" and "reject every other semantic binding of ... `entrypoint`". Reject exact forbidden argument names everywhere but preserve non-argument request names; the only `entrypoint` exception is the exact protected `_assert_entrypoint_positional_identity` FunctionDef/parameter/three in-body loads/two direct call sites enumerated above. All near misses fail. The exact `for name, value in {...}.items()` target is an `ast.Tuple` in `Store` context and must be compared with that context rather than a `Load` tuple.
