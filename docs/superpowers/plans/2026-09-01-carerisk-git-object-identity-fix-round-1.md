...
Architecture C deliberately stops after the executable product, static, raw-object,
custody-leak, frozen-product, and one-file scope gates above. Do not add any
repo-local executable source auditor or candidate-controlled review proof. Such a
mechanism lets the candidate define the proof of its own safety and recreates the
retired self-policing architecture.

The load-bearing source-safety gate is a fresh external review performed against
the immutable controller-approved BASE and candidate commit. Its checklist and
adversarial probes are reviewer evidence held outside tracked candidate source;
they are not copied into `tests/`, another executable repository path, or a
candidate-controlled self-authentication helper. Hash identity proves which bytes
were reviewed; it does not prove that those bytes are safe.
...
The fresh reviewer receives immutable controller values for BASE and candidate,
the exact reviewed plan, and both original repros. This review is load-bearing;
repository-local source cannot authenticate itself. The reviewer must:

1. verify the exact `BASE..candidate` diff changes only `tests/test_hf_space_source_boundary.py`, then read that complete diff plus the complete candidate definitions of `_sanitized_git_environment`, `_resolved_git_executable`, `_run_git_process`, `_resolve_git_repository`, `_git_bytes`, and `_init_temporary_bare_repository`;
2. trace executable process use directly in the candidate source and confirm there are no additional subprocess execution APIs or imported aliases, while the sole Git runner uses the resolved executable, sanitized environment, explicit repository binding, captured binary output, `check=True`, no shell, and exactly `timeout=10.0`;
3. trace every Git call site, including discovery, object reads, `hash-object`, and temporary bare SHA-1 initialization, and confirm each path receives the sanitizer and bounded runner before topology and object-format validation;
4. confirm no inherited uppercase `GIT_` or `CARERISK_` authority reaches Git, no fallback or tuple value exists in tracked executable source, and `_gradio_test_source_violations`, `_guard_helper_violations`, and any successor self-policing definition remain absent;
5. confirm constant no-context failure handling covers timeout, process failure, empty/multiline/non-ASCII typed metadata, and Unicode decode failure without exposing arguments, paths, stdout, stderr, environment, cause, or custody; and
6. confirm the five new cases, prior 13 Architecture C cases, full boundary/export and target suites, static checks, raw-object proof, custody leak checks, frozen-product diff, and exact one-file scope evidence all remain intact.

Using an independently controlled review package outside tracked candidate source,
the reviewer must also run and retain adversarial evidence for: hostile ambient
`GIT_DIR`/`GIT_WORK_TREE` and mixed-case routing variables; a missing timeout and
a non-10-second timeout; timeout, process-error, and Unicode/typed-metadata
failures; sanitizer replacement and runner environment/binding mutation; and
executable subprocess aliases including `run`, `Popen`, `call`, `check_call`, and
`check_output`. These disposable probes are reviewer evidence only. They must not
be copied into tracked candidate files or become a repository-local self-scanner.

Acceptance requires Spec ✅, Quality Approved, Critical `0`, Important `0`, the
external direct-source checklist and adversarial evidence above, followed by an
independent controller rerun of Steps 5 and 6. Any finding stops before ledger
release. This plan introduces no repository-local self-authentication and does not
authorize another fix round, Task 7, push, deployment, remote metadata, or
target-file changes.
