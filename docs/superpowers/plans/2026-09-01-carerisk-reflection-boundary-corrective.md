...
- Existing unrelated test introspection in `space/tests/test_gradio_contract.py`, such as reading `original_router` or `AF_UNIX`, is not an application capability and must remain valid. Guard-helper classification builds bounded module-level seeds from imports and ordinary assignments, rejects module-owned applications before crossing a function boundary, passes the state into each function, and extends it locally for sensitive roots, `__builtins__`/`builtins` aliases, immediate or named builtins mapping expressions, recursively composed attribute/subscript/`.get(...)` callables, tracked callable `.__call__`, and further ordinary aliases; it does not interpret other containers, defaults, returns, or control flow.
...
The fixed point is limited to relevant import tokens and ordinary `Name` assignments plus recursively composed tracked mapping/module attribute, `[...]`, `.get(...)`, and callable `.__call__` sources; it does not evaluate arbitrary mappings or expressions. Add literal and nonliteral key variants for immediate `b.__dict__` and `vars(b)` subscript/`.get(...)`, plus module-owned equivalents. The module pre-pass must itself reject the two top-level `guard = ...` applications rather than relying on the later use of the reflected result. The `b.__dict__` mutation proves that a tracked builtins-module dunder dictionary is a mapping source, not a callable source; `vars` ordinary aliases must preserve the equivalent mapping transition. Direct, two-level alias, module-owned, tracked builtins-attribute, and named-helper `.__call__` variants are mandatory. The earlier mutations prove that module-scope mapping, callable, import, and sensitive-root aliases are inherited by the function audit. Add equivalent function-local `import builtins as builtin_map` and `from builtins import getattr as reflect` mutations. Preserve the existing positive test for direct bounded builder/guard aliases. Add one explicit acceptance test whose functions contain only `getattr(other, "get")`, `getattr(inner, "original_router", None)`, and `getattr(socket, "AF_UNIX", None)` and assert `_guard_helper_violations(tree) == []`; a sensitive word on an unrelated receiver is not helper candidacy.
...
3. `_guard_helper_violations` performs a fail-closed candidate pre-pass before `if not all_guard_calls: continue`. Build a bounded alias state `(sensitive_aliases, builtin_module_aliases, builtin_mapping_aliases, reflection_callable_aliases, vars_callable_aliases)` from module-owned imports and ordinary assignments while excluding nested function/class bodies. Seed sensitive roots with `ui_module`, `gr`, `uvicorn`; seed builtin modules with effective local names from `import builtins`; seed mappings with `__builtins__` and any import whose original name is `__builtins__`; seed reflection callables with direct forbidden builtin/helper names plus effective aliases from `from builtins import <forbidden name>`; seed the `vars` subset with `vars` and effective aliases from `from builtins import vars`. Iterate exact `Name` assignment propagation to a fixed point, propagating the `vars` subset independently as well as the general reflection-callable set.

Implement `_mapping_source(expression, state)` and `_reflection_callable_source(expression, state)` as mutually bounded expression classifiers over an exact grammar, not as general evaluation. `_mapping_source` accepts a tracked mapping `Name`; `builtins_alias.__dict__`; a one-argument, no-keyword call of a tracked `vars_callable_alias` to a tracked builtins module; and a tracked reflection-call result applied to a builtins module when that result is subsequently consumed as a mapping. `_reflection_callable_source` accepts a tracked callable `Name`; a non-`__dict__` attribute of a tracked builtins module; a subscript whose receiver satisfies `_mapping_source`; `.get(...)` whose receiver satisfies `_mapping_source`; `.__call__` whose receiver recursively satisfies `_reflection_callable_source`; a call whose callee recursively satisfies `_reflection_callable_source` and whose first argument is a tracked builtins module; and the already named forbidden helpers. Recursion is limited to these node types and must terminate by descending the AST; it does not follow arbitrary calls or containers. Assignment targets inherit mapping/callable/`vars` state from these classifiers, so `v = vars; mapping = v(b)`, `mapping = b.__dict__`, `call = reflect.__call__`, and further exact `Name` aliases converge in the fixed point. Extend a copy of inherited state over each function's owned imports and assignments, including its `parent` sensitive root, before auditing calls. Module-level `sensitive_alias = ui_module` therefore remains sensitive inside the helper, and module-level mapping/callable aliases cannot disappear at the function boundary.

Factor the call check so it can audit module-owned statements as well as one function. Immediately after constructing module state, scan the module-owned statements (still excluding nested function/class bodies) and append `module:sensitive_reflection` if a direct or tracked callable, including an immediate recursively classified mapping subscript/`.get(...)` or callable `.__call__` expression, is applied to a module-sensitive first argument. Only then start per-function audits. `_sensitive_reflection_in_helper` applies the same call predicate to the inherited-plus-local state. Literal and nonliteral mapping keys both fail closed in that application. The fixed point does not follow defaults, returns, other containers, control flow, or arbitrary expressions, and it does not recursively enter a nested scope while constructing or auditing the parent scope. A literal member string without a sensitive receiver is insufficient. A nonliteral member on a sensitive receiver fails. The accepted unrelated receiver mutations are mandatory regression coverage.
