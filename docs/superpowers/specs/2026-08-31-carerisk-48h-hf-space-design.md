...
The Gradio test-source contract rejects every load, import original/effective name, or semantic binding of the forbidden reflection builtins, including `help` and `dir`; `__builtins__`/`builtins`; `operator.attrgetter`/`methodcaller`; `inspect.getattr_static`, `getmembers`, and equivalent dynamic inspect members; `importlib.import_module`; and every non-allowlisted reflection-bearing dunder attribute. The two exact `getattr` callee nodes are the sole builtin-name-load exceptions. The identities `getattr`, `type`, `isinstance`, `super`, `frozenset`, `inspect`, `importlib`, `socket`, `pytest`, `ui_module`, `gr`, and `uvicorn` cannot be rebound or import-laundered outside their exact existing imports and contexts.
...
The preceding closed-world rules have two actual-source clarifications that govern over broader wording. First, the accepted pytest decorator forms include both bare `@pytest.fixture` and exact current `@pytest.fixture(scope="module")`. Every `ast.arg` named exactly `request` or `pytestconfig` is rejected, including collected class methods and fixtures; this does not reject current nested helper function names, local/loop variables, methods, `request_bytes`, or other non-argument uses of the word `request`. Second, the dynamic-module local `entrypoint` remains protected as specified, but a separate exact exception permits the first parameter `entrypoint: Any` of the sole top-level helper `_assert_entrypoint_positional_identity`. That FunctionDef must be unique, top-level, undecorated, non-generic, retain its exact current signature, and contain exactly the current five direct `Assert` statements as its complete body in current order: the parent, demo, and app identity assertions, the exact `isinstance(served_app, ui_module.PublicSurfaceGuard)` assertion, and the downstream-parent identity assertion. Its name may be loaded only as the direct callee at the two current exact call sites in `test_entrypoint_mount_and_uvicorn_contract_are_exact`. Wrong owner, header, decorator, position, annotation, body count/order/context, nested control flow, duplicate, alias, alternate attribute/call site, added use, import, delete, global/nonlocal, or any rebind fails.

Every protected identity load is denied before exact exceptions are applied; checking only bindings or the existence of required nodes is insufficient. `inspect`, `importlib`, `uvicorn`, `gr`, `super`, `socket`, and `sys` use finite allowed-node sets constructed by exact owner/receiver/parent/call recognizers and required cardinalities from the reviewed current source. An assignment or capture such as `alias = inspect`, `api = importlib`, `runner = uvicorn`, or `mp = monkeypatch` fails at the protected source `Name` load. Extra direct calls fail even when one required call still exists. Receiver-independent loader member tokens `import_module`, `load_module`, and `load` are forbidden; `spec_from_file_location`, `module_from_spec`, `exec_module`, `signature`, `Parameter.empty`, `Config`, `Server`, and `mount_gradio_app` exist only in their already enumerated exact nodes. `ui_module` may load only as the immediate receiver of a direct attribute token or as the exact first argument of an already approved direct `monkeypatch.setattr` node, after which the sensitive guard/builder rules still apply; it cannot be captured as a bare value. Direct builtin `isinstance` loads remain limited to direct calls. `frozenset` loads remain limited to direct calls, exact type-annotation subscripts, or the exact second type operand of a direct `isinstance` call, never alias sources.

The complete semantic dunder-binding audit used by application source also governs the Gradio contract: every dunder Store/Del name, parameter, function/class name, import original/effective alias, exception target, pattern capture, and global/nonlocal declaration fails except class-owned `__init__`/`__call__`. Mutation API member strings reject every dunder literal, not only a short protocol subset, so `"__class__"`, `"__dict__"`, `"__globals__"`, `"__getitem__"`, and `"__call__"` all fail. Every `monkeypatch` load is either the exact receiver root of one approved direct `setattr` or the one exact `setenv` call, or one of eight finite helper arguments already present in the positive source: argument index 2 of three exact `make_unit_failure_bundle` calls and argument index 0 of five exact `_fake_asset_roots` calls, each pinned by owner, callee, position, and cardinality. Both callees are unique top-level undecorated, non-generic exact-signature FunctionDefs whose names can load only at those exact call sites; their exact `monkeypatch: pytest.MonkeyPatch` parameters load internally only as receivers of their current two direct `setattr` calls. Method capture, aliases, returns, helper decoration/rebinding/import/renamed parameters, alternate internal methods, wrong helper/position/owner, and extra occurrences fail before method interpretation. `sys` has exactly two allowed load nodes: the reviewed `sys.platform == "win32"` comparison and the exact `monkeypatch.setattr(sys, "platform", "linux")` target. Every other `sys` load and every direct `platform` Store/Del fails.

Protected module identities cannot be reacquired through attributes. An attribute token named `inspect`, `importlib`, `socket`, `sys`, `pytest`, `ui_module`, `gr`, `uvicorn`, `Path`, `SPACE_ROOT`, or `AppEntryMarker` is rejected independently of receiver. There are exactly two exceptions. The first is the exact `entrypoint.uvicorn` node used as argument 0 of the canonical `monkeypatch.setattr(entrypoint.uvicorn, "run", fake_run)` call. The second is the exact `socket.socket` callee in the sole direct call `socket.socket(unix_family, socket.SOCK_STREAM)` owned by `test_package_asset_special_file_fails_closed`; that call occurs once, has exactly those two positional arguments, and has no keywords. Thus `ui_module.gr`, `entrypoint.gr`, `runner = entrypoint.uvicorn`, captured or alternate-receiver `.socket` nodes, a second `socket.socket(...)` call, and re-export paths such as `evidence_module.Path` fail at their source token.

The loader chain is one exact ordered four-statement direct-body sequence in `test_entrypoint_mount_and_uvicorn_contract_are_exact`: assign `spec` from the pinned location call; assert `spec is not None and spec.loader is not None`; assign `entrypoint = importlib.util.module_from_spec(spec)`; and call `spec.loader.exec_module(entrypoint)`. Every `spec` Name load and `spec.loader` Attribute is a node inside that sequence and no other load/member/capture is permitted. Loader capabilities including `get_code`, `get_data`, `create_module`, and another `exec_module` fail receiver-independently.

All 26 committed `monkeypatch.setattr` calls are complete canonical AST contracts, including target, member, replacement expression, owner, and call shape; a replacement such as `os.system` is not accepted merely because target/member match. The sole `setenv` contract remains separate and exact. The inspect guard contract is the complete direct assignment `parameters = inspect.signature(ui_module.PublicSurfaceGuard).parameters`, and `inspect.Parameter.empty` exists only in its complete current direct Assert parent. The accepted `super().__init__(level=logging.DEBUG)` is the exact first direct statement of the exact `BoundedLogCapture.__init__` body; nesting it in `if`, `try`, lambda, or another control-flow context fails.
