...
`monkeypatch` exists only as an exact `pytest.MonkeyPatch` parameter owned by the function that uses it and cannot be rebound. `monkeypatch.setattr(...)` remains a test mutation API only as an exact direct attribute callee with a literal member name; it is not a builtin `setattr` exception and may not be aliased. It cannot replace an allowlist identity anchor such as `inspect.signature`/`Parameter`, `gr.__version__`, `socket.AF_UNIX`, the permitted `importlib.util` members, or `ui_module.PublicSurfaceGuard`. The four existing entrypoint-contract substitutions of `ui_module.create_app`, `ui_module.build_package_asset_membership`, `gr.mount_gradio_app`, and `entrypoint.uvicorn.run` are exact function-scoped exceptions. Existing unaliased `inspect` and `importlib.util` imports remain permitted only for their enumerated static contract uses.
