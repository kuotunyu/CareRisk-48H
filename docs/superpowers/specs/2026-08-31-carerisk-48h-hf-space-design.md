...
For test-helper classification only, bounded fixed-point lineage is computed first over module-level imports and ordinary assignments, then extended inside each audited function. The inherited and local state resolves ordinary aliases for sensitive roots; the implicit `__builtins__` mapping; `import builtins` aliases; imported `__builtins__` aliases; direct forbidden builtin callables; `from builtins import <forbidden>` aliases; mappings obtained as `builtins_alias.__dict__` or `vars(builtins_alias)`; callables obtained from a tracked mapping/module through attribute access, a subscript, or `.get(...)`; and further ordinary name aliases of those mappings and callables. A nonliteral mapping key fails closed when the resulting callable is applied to a sensitive root.
