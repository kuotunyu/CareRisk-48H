...
The complete semantic dunder-binding audit used by application source also governs the Gradio contract: every dunder Store/Del name, parameter, function/class name, import original/effective alias, exception target, pattern capture, and global/nonlocal declaration fails except class-owned `__init__`/`__call__`. Mutation API member strings reject every dunder literal, not only a short protocol subset, so `"__class__"`, `"__dict__"`, `"__globals__"`, `"__getitem__"`, and `"__call__"` all fail. Every `monkeypatch` load is the exact receiver root of one approved direct `setattr` or the one exact `setenv` call; method capture and aliases fail before method interpretation. `sys` has exactly two allowed load nodes: the reviewed `sys.platform == "win32"` comparison and the exact `monkeypatch.setattr(sys, "platform", "linux")` target. Every other `sys` load and every direct `platform` Store/Del fails.
