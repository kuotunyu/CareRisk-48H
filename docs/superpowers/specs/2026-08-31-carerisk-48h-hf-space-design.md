...
The reviewer image is ephemeral local/CI verification infrastructure. It may be pulled and run by exact digest, but it is never saved, exported, emitted as a build output, pushed, uploaded, published, deployed, copied into a public candidate, or used as an ancestor/layer/filesystem source for the final runtime stage. The SBOM and license inventory may carry the tuple's metadata-only record because those records make the exclusion auditable; they do not carry the browser bytes or upstream source file. A controlled-acquisition phase alone may HTTPS GET the exact commit-pinned raw URL and validate its raw byte length/hash and the three exact assignments. Its only retained result is bounded parsed canonical metadata/evidence at a verified GUID-owned OS-temp child, never a repository, runtime, export, or public-bundle file; downstream no-egress verification reads that metadata through an explicit offline/network-bomb interface, and cleans only the exact owned temp child after outputs/receipts succeed. For the current Task 7 correction, the already measured `base-image.json` tag/index/manifest/browser-tree values are frozen inputs and may only receive the exact provenance/version fields in this section. The correction runs a read-only `verify-images` gate and must not run `resolve-images` or overwrite/reselect the image. Any future legitimate re-resolution is a new tuple and requires separate central design approval, fresh measurement/license review, mutation updates, and a new custody baseline before changing this record.
