...
Git plumbing has no ambient repository or timing authority. Every Git subprocess launched by `tests/test_hf_space_source_boundary.py` uses a resolved absolute executable, a deterministic minimal environment that drops all inherited `GIT_*` and `CARERISK_*` keys, fixed nonsecret Git/locale controls, captured binary output, `check=True`, and a 10-second timeout. Under that sanitized environment, verification resolves the requested path, absolute Git directory, and bare/non-bare identity; a non-bare top level must equal the requested repository and a bare Git directory must equal it. Every later object command is explicitly bound to the resolved Git directory, with the resolved requested directory as work tree only for non-bare repositories. Temporary bare repositories use exact SHA-1 initialization under the same bounded runner and prove bare identity plus object format before writing an object. Timeout, command, path-resolution, and Unicode-metadata failures expose only the constant nonsecret message `bounded Git plumbing failed`, never command arguments, paths, stdout, stderr, environment, or custody.
