...
Protected module identities cannot be reacquired through attributes. An attribute token named `inspect`, `importlib`, `socket`, `sys`, `pytest`, `ui_module`, `gr`, `uvicorn`, `Path`, `SPACE_ROOT`, or `AppEntryMarker` is rejected independently of receiver. The sole exception is the exact `entrypoint.uvicorn` node used as argument 0 of the canonical `monkeypatch.setattr(entrypoint.uvicorn, "run", fake_run)` call. Thus `ui_module.gr`, `entrypoint.gr`, `runner = entrypoint.uvicorn`, and re-export paths such as `evidence_module.Path` fail at their source token.
