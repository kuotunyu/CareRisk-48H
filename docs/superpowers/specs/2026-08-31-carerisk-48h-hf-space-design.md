...
The public bundle's Gradio executable-contract test is governed by a qualitatively distinct external identity boundary, not by a scanner embedded in another candidate-controlled test file. The frozen path is `space/tests/test_gradio_contract.py`. Its reviewed Git object is blob SHA-1 `7c75d61c53eccdc93f69e7e3bb1eb09346eb5f04`, with raw Git-object size `64847` bytes and raw Git-object SHA-256 `101893daf6f20f9b507a00d0ac8da7fa383f83007520b4db61b710d1814df2a8`. Verification resolves `HEAD:space/tests/test_gradio_contract.py`, requires that exact object identity and type, and computes size and SHA-256 only over the exact bytes emitted by `git cat-file blob`. Checkout bytes, `Path.read_bytes`, PowerShell `Get-Content`, text-mode reads, and newline normalization are not identity authorities.

The expected tuple is controller custody. For every later local task that invokes `tests/test_hf_space_source_boundary.py`, the controller reads the ignored task brief/custody ledger and injects exactly `CARERISK_GRADIO_CONTRACT_BLOB_SHA1`, `CARERISK_GRADIO_CONTRACT_RAW_SIZE`, and `CARERISK_GRADIO_CONTRACT_RAW_SHA256` into that task process. No tracked test constant, fallback, repository manifest, deployment manifest, generated candidate file, or candidate-controlled parser may provide or update the expected values. The tracked design records the reviewed identity for governance, but implementation must fail if any custody value is absent or malformed rather than reading this document. `tests/test_hf_space_source_boundary.py` may contain only the generic Git-object reader, strict custody-input validation, comparison logic, and mutation tests. It must not contain `_gradio_test_source_violations`, `_guard_helper_violations`, a successor Gradio-source meta-scanner, or supporting allowlists that attempt to interpret the target test file.

Git plumbing has no ambient repository or timing authority. Every Git subprocess launched by `tests/test_hf_space_source_boundary.py` uses a resolved absolute executable, a deterministic minimal environment that drops all inherited `GIT_*` and `CARERISK_*` keys, fixed nonsecret Git/locale controls, captured binary output, `check=True`, and a 10-second timeout. Under that sanitized environment, verification resolves the requested path, absolute Git directory, and bare/non-bare identity; a non-bare top level must equal the requested repository and a bare Git directory must equal it. Every later object command is explicitly bound to the resolved Git directory, with the resolved requested directory as work tree only for non-bare repositories. Temporary bare repositories use exact SHA-1 initialization under the same bounded runner and prove bare identity plus object format before writing an object. Every typed metadata result—object type, raw size, written object ID, object format, positive path-object ID, and bare token—must pass through one strict nonempty single-line ASCII decoder; absolute Git-directory and top-level filesystem paths use the separate strict filesystem-path decoder. Timeout, command, path-resolution, invalid-ASCII, empty, multiline, and non-ASCII typed-metadata failures expose only the constant nonsecret message `bounded Git plumbing failed`, never command arguments, paths, stdout, stderr, environment, custody, cause, context, or a raw decode exception.

Task 11 may reference the three custody names through a separately authorized GitHub Actions Environment and external Actions variables. The tracked workflow may contain only `${{ vars.CARERISK_GRADIO_CONTRACT_BLOB_SHA1 }}`, `${{ vars.CARERISK_GRADIO_CONTRACT_RAW_SIZE }}`, and `${{ vars.CARERISK_GRADIO_CONTRACT_RAW_SHA256 }}`, never their values, and passes them into the read-only reviewer container with exact same-name `--env` arguments. Workflow contract tests require the exact three-name transfer, reject every tracked occurrence of the controller-supplied values outside governance docs, and prove missing or malformed Actions values fail before tests. Creating the Actions Environment or setting/changing its variables is remote metadata mutation and remains blocked until separate written authorization; no local Task 11 commit grants that authority.

Future Task 11 also owns a separate reviewer-checkout ownership boundary; this
does not change current Task 6 acceptance. The reviewer container must either
run as the host checkout owner's exact numeric UID/GID or receive an
independently controller-resolved exact path for the read-only mounted checkout
as its sole `safe.directory`. Workflow contract tests prove the selected mode,
the exact UID/GID or byte-exact resolved path transfer, and fail-closed behavior
for missing, malformed, mismatched, sibling/parent, multiple, or wildcard
values. `safe.directory=*`, candidate-controlled or repository-local Git
configuration, a candidate-provided resolution result, and global/system config
mutation are forbidden. When the path mode is selected, equality with the
independently resolved mounted checkout is established before Git or pytest and
only that exact path is passed by command scope or an isolated
controller-created protected config. Any ownership/config ambiguity stops the
reviewer before tests; Task 11 remote metadata operations remain separately
authorization-gated.

Git-object identity is provenance, not a safety proof. Before the tuple is accepted, a fresh reviewer must inspect the exact `git cat-file blob` bytes directly, review the executable assertions and their placement, and report Critical `0` and Important `0`. Only after the identity gate proves that a clean tracked checkout's `HEAD:space/tests/test_gradio_contract.py` resolves to that reviewed object may pytest import and execute the checkout path. Pytest never executes a pipe of raw object bytes or a temporary materialization, and checkout reads are execution semantics rather than identity authority. Application-source AST/import/capability scanners, exporter/public-path boundaries, product frozen-byte gates, and later container/browser gates remain independently authoritative; this architecture removes only the failed attempt to make one test file police another test file's Python semantics.

Any legitimate future change to `space/tests/test_gradio_contract.py` uses two phases. Phase A changes only that target file in a standalone commit while the old external identity remains active, so the identity gate must fail. A fresh reviewer inspects the new raw Git object and its executable test behavior; the controller derives the candidate tuple with `git cat-file`, records approval outside tracked candidate content, and rejects the phase if any Critical or Important finding remains. Phase B updates controller custody to the approved tuple and, only if the generic boundary mechanism itself must change, does so in a separate commit that cannot also modify the target file. The controller then reruns identity, target-suite, full boundary/export, static, scope, and frozen-product gates. A target change and its expected tuple can never be self-approved in one candidate or one review phase.

For the current migration, `space/tests/test_gradio_contract.py` remains byte-identical to the frozen object. Implementation changes only `tests/test_hf_space_source_boundary.py`: remove both retired self-policing functions, their solely supporting constants/helpers, and their mutation tests; add the generic external identity contract and eight temporary-Git-object mutation categories. Task 7 remains blocked until direct raw-object review, independent corrective review, controller reruns, clean scope, and explicit ledger reconciliation accept the new boundary.
...
- Every component whose bytes enter the clean public export, candidate context, runtime stage, final runtime image, deployment artifact, saved image/archive, pushed image, uploaded artifact, published image, emitted build output, or other distributed output must have an approved redistribution-compatible policy record. Unknown, missing, incompatible, or non-redistributable licensing for any such byte is a hard export stop.
...
- WebKit revision, browser version, and immutable canonical tree identity: revision `2336`, version `26.5`, exactly `38` files and `306401261` bytes, algorithm `sha256-canonical-tree-v1`, SHA-256 `c9df99c2d0597f5c9d6bc8084a83c6ab9e929a17282859bee951cedc87562c8c`.
- Image provenance references remain tag `https://github.com/microsoft/playwright/tree/v1.62.0`; tagged browser map `https://github.com/microsoft/playwright/blob/v1.62.0/packages/playwright-core/browsers.json`; tagged registry source `https://github.com/microsoft/playwright/blob/v1.62.0/packages/playwright-core/src/server/registry/index.ts`; and resolved official CDN artifact `https://cdn.playwright.dev/dbazure/download/playwright/builds/webkit/2336/webkit-ubuntu-24.04.zip`.
- The separately acquired, metadata-only Playwright source reference is exact: `playwright_tag=v1.62.0`; `playwright_tag_commit=e3950d9c140d007bd52853b45813c6274b24e36f`; `repository_relative_path=browser_patches/webkit/UPSTREAM_CONFIG.sh`; `commit_pinned_raw_url=https://raw.githubusercontent.com/microsoft/playwright/e3950d9c140d007bd52853b45813c6274b24e36f/browser_patches/webkit/UPSTREAM_CONFIG.sh`; `raw_byte_length=126`; `raw_sha256=3554c5b666ed87032fb22e78956f8a2fffe1faede63ae8dcae60a26961f6419c`; parsed `REMOTE_URL=https://github.com/WebKit/WebKit.git`; `BASE_BRANCH=main`; and `BASE_REVISION=343e13bf22dca9d0ec227801419aab0f9001a32f`.
...
The WebKit policy record must use `licenseDeclared="NOASSERTION"`, `licenseConcluded="NOASSERTION"`, `review_disposition="reviewer_test_only_not_redistributed"`, and `complete_digest_bound_notice=false`. The license inventory must preserve those exact semantics; the SPDX package entry must preserve the exact declared and concluded values. The external source reference proves only that Playwright `v1.62.0` source configuration declares this WebKit base revision. It does not claim that the file is in the binary image, bitwise source-to-binary attestation, a complete digest-bound notice set, redistribution approval, or a guessed SPDX expression.

The exception is fail closed. Any drift in image tag, index digest, linux/amd64 manifest, Playwright version/tag/tag URL, `browsers.json`, registry/CDN reference, WebKit revision/version/typed canonical tree count/typed total bytes/tree algorithm/tree identity, any `playwright_tag_commit`, `repository_relative_path`, `commit_pinned_raw_url`, raw length/hash, parsed `REMOTE_URL`, `BASE_BRANCH`, or `BASE_REVISION`, official licensing-reference set, exact `NOASSERTION` fields, disposition, or `complete_digest_bound_notice=false` stops verification. The image-tree absence value must be recomputed from the canonical inventory, alongside count/bytes/algorithm/digest, and cannot be a self-asserted boolean; source metadata must never be represented as a distributed image byte. Before any Task 7 test, the controller—outside product and future supply-chain-generator code—performs exactly one normal-TLS HTTPS GET to the fixed commit-pinned URL. Raw bytes exist only in the exact GUID-owned OS-temp child long enough to validate length, SHA-256, and the three assignments; only bounded parsed metadata/evidence is then retained in the exact metadata file. All later verifier/inventory/test commands require that exact reference, matching run GUID and phase, plus literal offline/network-bomb flags; they load and validate the typed reference and install the bomb before any work or child process. Missing or alternate path/GUID/phase/flag/network-permission metadata fails closed. Existing measured locks/images are frozen inputs: this correction does not re-acquire dependency locks or re-resolve/reselect images. A Task 13 receipt serializes only the sanctioned policy tuple, absence proof, and bounded acquisition/lifecycle evidence, never the transient source-reference path, raw bytes, runner flags, or run GUID. Any WebKit or reviewer-image byte in the public export, candidate context, runtime stage, final image, deployment artifact, saved archive, pushed image, uploaded artifact, published image, emitted build output, or any other distributed output also stops verification. Chromium, Firefox, ffmpeg, both operating-system inventories, both base-image records, and every Python component remain governed by the ordinary approved-only rule.

The distribution validator uses the closed surface set `public_export`, `candidate`, `runtime_stage`, `final_image`, `deployment_artifact`, `saved_archive`, `pushed_image`, `uploaded_artifact`, `published_image`, `build_output`, and `other_distributed_output`. Each member receives an independent reviewer/browser-byte mutation. An omitted, duplicate, unknown, empty, or unclassified surface fails closed before validation; `other_distributed_output` is the mandatory catchall for a real distribution mechanism not covered by the ten named mechanisms, not permission to skip inspection.

The reviewer image is ephemeral local/CI verification infrastructure. It may be pulled and run by exact digest, but it is never saved, exported, emitted as a build output, pushed, uploaded, published, deployed, copied into a public candidate, or used as an ancestor/layer/filesystem source for the final runtime stage. The SBOM and license inventory may carry the tuple's metadata-only record because those records make the exclusion auditable; they do not carry the browser bytes or upstream source file. The controller-owned controlled-acquisition phase alone may HTTPS GET the exact commit-pinned raw URL once and validate its raw byte length/hash and the three exact assignments. It writes the response only to the task-owned transient OS-temp child, deletes that raw file after validation through the same ownership validator used by all Task 7 cleanup paths, and retains only bounded parsed canonical metadata/evidence in the exact source-reference file. Downstream no-egress verification reads that metadata through an explicit source-reference/run-GUID/phase/offline/network-bomb interface. The Task 0 catch path and the success finalizer call one shared ownership validator that revalidates the exact OS-temp root, resolved-child containment, candidate-not-root, exact run-GUID-derived name, non-reparse/non-symlink identity, and phase-appropriate expected child set before any removal; ambiguity retains the target untouched. For the current Task 7 correction, the already measured `base-image.json` tag/index/manifest/browser-tree values are frozen inputs and may only receive the exact provenance/version fields in this section. The correction runs a read-only `verify-images` gate and must not run `resolve-images` or overwrite/reselect the image. Any future legitimate re-resolution is a new tuple and requires separate central design approval, fresh measurement/license review, mutation updates, and a new custody baseline before changing this record.
...
- Space card metadata, license boundary, citation, NOTICE, SBOM, and one-license-record-per-lock-package tests. Supply-chain mutation coverage must reject `NOASSERTION` on every public/deployed component, `NOASSERTION` on any non-WebKit component, every single-field drift of the exact Section 10.1 tuple/provenance/exclusion record (including tree algorithm and official tag URL), `complete_digest_bound_notice=true`, a guessed WebKit license expression, and any reviewer/WebKit/browser byte entering each of the eleven closed distribution surfaces. Separate mutations must reject a missing, duplicate, unknown, empty, or unclassified surface.
...
- Treat the reviewer image as ephemeral local/CI-only infrastructure. The verifier may pull and run its exact digest, but every command plan and image-history/export inspection must prove that the reviewer image and its Chromium/Firefox/WebKit/ffmpeg bytes are absent from all eleven closed distribution surfaces, including save, export, push, upload, publication, and build-output paths. The metadata-only SBOM/license records remain required.
...
- Every distributed/public/runtime byte has an approved redistribution-compatible policy disposition. The sole exact WebKit reviewer tuple is present only as an SBOM/license metadata record with `licenseDeclared="NOASSERTION"`, `licenseConcluded="NOASSERTION"`, `review_disposition="reviewer_test_only_not_redistributed"`, and `complete_digest_bound_notice=false`; its complete tuple/provenance/exclusion fields, including tree algorithm and official tag URL, match Section 10.1 and no reviewer/browser byte reaches any of the eleven closed distribution surfaces.
