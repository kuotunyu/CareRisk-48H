...
For test-helper classification only, bounded fixed-point lineage is computed first over module-level imports and ordinary assignments, then extended inside each audited function. The inherited and local state resolves ordinary aliases for sensitive roots; the implicit `__builtins__` mapping; `import builtins` aliases; imported `__builtins__` aliases; direct forbidden builtin callables; `from builtins import <forbidden>` aliases; mappings obtained as `builtins_alias.__dict__`, `vars_alias(builtins_alias)`, or the equivalent immediate tracked expression; callables obtained from a tracked mapping/module through attribute access, a subscript, or `.get(...)`; the `.__call__` attribute of any tracked reflection callable; and further ordinary name aliases of those mappings and callables. Mapping/callable classification composes recursively over only those exact expression forms, so a receiver does not need an intermediate assignment. A nonliteral mapping key fails closed when the resulting callable is applied to a sensitive root.

The module-owned pre-pass does not merely harvest state: before any per-function audit, it rejects a direct or tracked reflective callable application whose first argument is a module-sensitive root, including an application embedded in a top-level assignment. The analysis does not follow defaults, returns, other containers, control flow, or arbitrary expressions, and it does not apply the product-source ban wholesale to unrelated test introspection. A tracked callable applied to an inherited or local sensitive-root alias is always a candidate. A sensitive word on an unrelated receiver is insufficient. Existing test-only introspection such as `getattr(inner, "original_router", None)` and `getattr(socket, "AF_UNIX", None)` remains outside this policy.
