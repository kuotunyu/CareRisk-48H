...
The preceding closed-world rules have two actual-source clarifications that govern over broader wording. First, the accepted pytest decorator forms include both bare `@pytest.fixture` and exact current `@pytest.fixture(scope="module")`. A `request` or `pytestconfig` name is rejected only when it is a fixture-injectable parameter of a top-level collected test or top-level exact-fixture-decorated function; existing nested helper names, local/loop variables, methods, and method parameters named `request` remain allowed. Second, the dynamic-module local `entrypoint` remains protected as specified, but a separate exact exception permits the first parameter `entrypoint: Any` of the sole top-level helper `_assert_entrypoint_positional_identity`. Its only loads are the receivers in the three existing identity assertions `mounted_parent is entrypoint.parent`, `mounted_demo is entrypoint.demo`, and `served_app is entrypoint.app`; wrong owner, position, annotation, duplicate, alternate attribute, added use, or any rebind fails.
