...
依預註冊規則，GRU-D 與 TCN 未達到超越最佳 tabular model 的門檻，因此選用較簡單的 3-seed LightGBM ensemble，搭配 Platt calibration 與固定 threshold。Set B 僅成功評估一次，confidence interval 使用 2,000 次 stratified bootstrap。
...
```mermaid
flowchart TD
    A["Set A"] --> B["固定 split<br/>train / validation / calibration"]
    ...
    F --> G["freeze manifest"]
    G --> H["Set A simulated dry-run"]
    H --> I["Set B final evaluation<br/>唯一一次"]
...
Set A 資料準備與完整 tabular training：
...
- Set A 用於 development；Set B 在 freeze 後只 final evaluation 一次；Set C 完全不使用。
