...
<style>
  ...
  .cr-kicker { margin: 0 0 8px; color: var(--accent); font-weight: 700; letter-spacing: .08em; text-transform: uppercase; }
  .cr-shell h1 { margin: 0; font-size: clamp(30px, 5vw, 52px); line-height: 1.05; letter-spacing: -.035em; }
  .cr-subtitle { margin: 10px 0 0; color: var(--muted); font-size: 18px; }
  .cr-boundary { margin: 24px 0; padding: 16px 18px; border: 1px solid var(--line); border-left: 5px solid var(--accent); border-radius: 12px; background: var(--wash); }
  ...
  .cr-option { display: flex; align-items: center; gap: 10px; min-height: 48px; padding: 10px 12px; border: 1px solid var(--line); border-radius: 10px; cursor: pointer; background: var(--paper); }
  .cr-option:hover { border-color: var(--accent); }
  .cr-option:has(input:checked) { border-color: var(--accent); box-shadow: 0 0 0 2px rgba(34, 92, 112, .14); }
  .cr-option input { width: 22px; height: 22px; margin: 0; accent-color: var(--accent); flex: 0 0 auto; }
  ...
  .cr-panel { display: none; padding: clamp(18px, 3vw, 28px); border: 1px solid var(--line); border-radius: 14px; background: var(--wash); }
  .cr-panel h2 { margin: 0 0 10px; font-size: clamp(21px, 3vw, 28px); }
  .cr-panel p { margin: 0; line-height: 1.65; }
  .cr-panel p + p { margin-top: 12px; padding-top: 12px; border-top: 1px solid var(--line); color: var(--muted); }
  ...
  .cr-shell:has(#state-provenance_withheld:checked) .cr-panel[data-state="provenance_withheld"] { display: block; }
...
def render_explorer_html() -> str:
    ...
    return (
        _STYLES
        + '<section class="cr-shell" lang="zh-TW">'
...
def render_unavailable_html() -> str:
    ...
    return (
        _STYLES
        + '<section class="cr-shell" lang="zh-TW"><h1>CareRisk 48H</h1>'
        '<p>固定合成展示目前無法顯示。</p>'
        f"<p>{escape(SAFETY_ZH, quote=True)}</p>"
