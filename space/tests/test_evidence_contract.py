...
@pytest.mark.parametrize("needle", [
    b'"schema_version": 1', b'"events": 568', b'"n": 4000',
    b'"prevalence": 0.142', b'"set_b_final_evaluation_successes": 1',
    b'"samples": 2000', b'"seed": 2026',
    b'"threshold": 0.2974276505509685', b'"auroc": 0.8696003233855347',
    b'"lower": 0.8547766823845169',
])
def test_receipt_rejects_lexical_exponent_overflow(
    needle: bytes, monkeypatch: pytest.MonkeyPatch
) -> None:
    raw = receipt_raw()
    assert raw.count(needle) == 1
    value = needle.split(b": ")[1]
    raw = raw.replace(needle, needle.replace(value, b"1e9999"), 1)
