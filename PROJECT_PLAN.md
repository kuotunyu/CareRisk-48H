...
| 2026-08-04 | Set B final package integrity | strict README validator、freeze re-hash、outcome/ledger/lock hash、七個 artifact hashes、11 個預定 subgroup outputs 與四聯圖目視檢查 | 通過；metrics JSON 27,646 bytes/SHA-256 `808525afad2ec550e8059c4ba37c2f5aaf8af748873a5a590dff7f1aeaaf47af`；PR/ROC/reliability/decision curve、errors、subgroups、predictions、preflight、ledger/lock 全部存在且 hash 相符 |
| 2026-08-04 | README/cards formal result gate | 合格 metrics JSON 經 `scripts/update_readme_results.py` 自動更新 README；同步 MODEL_CARD、DATA_CARD 與 plan，targeted tests/hooks | 通過；26 tests passed；正式表與 cards 僅使用一次成功 Set B metrics，保留研究/教育、非臨床與不可遷移至長照聲明；commit `55a2b4a` |
